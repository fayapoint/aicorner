import { getUssAccess } from "@/lib/authz";
import { FeatureLocked } from "@/components/uss/feature-locked";

export default async function UssAnalyticsPage({
  searchParams,
}: {
  searchParams?: { [key: string]: string | string[] | undefined };
}) {
  const access = await getUssAccess();
  const embed = searchParams?.embed === "1" || searchParams?.embed === "true";

  if (!access.session || !access.email) {
    return (
      <div>
        <h1 className="text-2xl font-semibold mb-2">Analytics</h1>
        <p className="text-sm text-red-600">Você não está autenticado.</p>
      </div>
    );
  }

  if (!access.features.analytics) {
    return <FeatureLocked title="Analytics" />;
  }

  return (
    <div className={embed ? "space-y-3 p-2" : "space-y-4"}>
      {!embed && <h1 className="text-2xl font-semibold">Analytics</h1>}
      <p className="text-sm text-gray-600">Em breve: gráficos e KPIs da sua conta USS.</p>
    </div>
  );
}
